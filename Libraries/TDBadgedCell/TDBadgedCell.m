...
@synthesize width, badgeString, parent, badgeColor, badgeColorHighlighted, badgeTextColor, badgeTextColorHighlighted;
...
{	
	...
	CGRect bounds = CGRectMake(0 , 0, numberSize.width + 16 , 20);
	...
	bounds.origin.y = (bounds.size.height - numberSize.height) / 2;
	
    if (parent.highlighted || parent.selected) {
		if (self.badgeTextColorHighlighted) {
			col = self.badgeTextColorHighlighted;
		} else {
			col = [UIColor clearColor];
		}
	} else {
		if (self.badgeColor) {
			col = self.badgeTextColor;
		} else {
			col = [UIColor clearColor];
		}
	}
...
@synthesize badgeString, badge, badgeColor, badgeColorHighlighted, badgeTextColor, badgeTextColorHighlighted;
...
- (void) layoutSubviews
{
	...
	if(self.badgeString)
	{
		...
		if (version <= 3.0)
		{
			badgeframe = CGRectMake(self.contentView.frame.size.width - (badgeSize.width+16), round((self.contentView.frame.size.height - 20) / 2), badgeSize.width+16, 20);
		}
		else
		{
			badgeframe = CGRectMake(self.contentView.frame.size.width - (badgeSize.width+16) - 10, round((self.contentView.frame.size.height - 20) / 2), badgeSize.width+16, 20);
		}
		...
		//set badge highlighted colours or use defaults
		if(self.badgeTextColorHighlighted)
			badge.badgeTextColorHighlighted = self.badgeTextColorHighlighted;
		else 
			badge.badgeTextColorHighlighted = self.textLabel.textColor;
		
		//set badge colours or impose defaults
		if(self.badgeTextColor)
			badge.badgeTextColor = self.badgeTextColor;
		else
			badge.badgeTextColor = self.textLabel.textColor;
